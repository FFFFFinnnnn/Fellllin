...
class PainterBase:
    ...
    def _save_stroke_params(self, v):
        ...
        np.savez(
            file_name + "_strokes.npz", x_ctt=x_ctt, x_color=x_color, x_alpha=x_alpha
        )
...
class Painter(PainterBase):
    def __init__(self, args):
        ...
        self.img_ = (
            cv2.cvtColor(self.img_, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
        )
        ...
        self.img_batch = utils.img2patches(
            self.img_, args.m_grid, self.net_G.out_size
        ).to(device)
    ...
    def _drawing_step_states(self):
        ...
        print(
            "iteration step %d, G_loss: %.5f, step_psnr: %.5f, strokes: %d / %d"
            % (
                self.step_id,
                self.G_loss.item(),
                acc,
        ...
        vis2 = utils.patches2img(self.G_final_pred_canvas, self.m_grid).clip(
            min=0, max=1
        )
...
class NeuralStyleTransfer(PainterBase):
    def __init__(self, args):
        ...
        if os.path.exists(args.vector_file) is False:
            exit(
                "vector file does not exist, pls check --vector_file, or run demo.py fist"
            )
        else:
        ...
        img_ = cv2.cvtColor(img_, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
        ...
        self.img_batch = utils.img2patches(
            self.img_, self.m_grid, self.net_G.out_size
        ).to(device)

        style_img = cv2.imread(args.style_img_path, cv2.IMREAD_COLOR)
        self.style_img_ = (
            cv2.cvtColor(style_img, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
        )
        self.style_img = cv2.blur(cv2.resize(self.style_img_, (128, 128)), (2, 2))
        self.style_img = (
            torch.tensor(self.style_img).permute([2, 0, 1]).unsqueeze(0).to(device)
        )
    ...
    def _save_style_transfer_images(self, final_rendered_image):
        ...
        plt.imsave(
            file_dir + "_style_img_" + self.style_img_path.split("/")[-1][:-4] + ".png",
            out_img,
        )
