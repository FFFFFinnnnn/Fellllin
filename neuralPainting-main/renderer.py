...
class Renderer:
    ...
    def create_empty_canvas(self):
        if self.canvas_color == "white":
            self.canvas = np.ones([self.CANVAS_WIDTH, self.CANVAS_WIDTH, 3]).astype(
                "float32"
            )
        else:
            self.canvas = np.zeros([self.CANVAS_WIDTH, self.CANVAS_WIDTH, 3]).astype(
                "float32"
            )
    ...
    def random_stroke_params_sampler(self, err_map, img):
        ...
        if self.renderer in ["markerpen"]:
            ...
            color = color + color
            alpha = _random_floats(0.98, 1.0, 1)
            self.stroke_params = np.array(
                x + wh + theta + color + alpha, dtype=np.float32
            )
        elif self.renderer in ["rectangle"]:
    ...
    def _draw_watercolor(self):
        ...
        self.foreground = np.zeros_like(
            self.canvas, dtype=np.uint8
        )  # uint8 for antialiasing
        ...
        for i in range(100):
            ...
            cv2.circle(
                self.stroke_alpha_map, (x, y), radius, alpha, -1, lineType=cv2.LINE_AA
            )
        ...
        self.foreground = np.array(self.foreground, dtype=np.float32) / 255.0
        self.stroke_alpha_map = (
            np.array(self.stroke_alpha_map, dtype=np.float32) / 255.0
        )
    ...
    def _draw_markerpen(self):
        ...
        for i in range(100):
            ...
            pt0 = utils.rotate_pt(
                pt=(x - radius, y - radius), rotate_center=ptc, theta=theta
            )
            pt1 = utils.rotate_pt(
                pt=(x + radius, y - radius), rotate_center=ptc, theta=theta
            )
    ...
    def _update_canvas(self):
        return self.foreground * self.stroke_alpha_map + self.canvas * (
            1 - self.stroke_alpha_map
        )
